...
// Load Evently API configuration
...
// Start MCP server
const server = new EventlySGRServer(apiClient)
server.run().catch((error) => {
    console.error('Failed to start Evently SGR MCP Server:', error)
    process.exit(1)
})

console.log('✅ Evently SGR MCP Server started')
