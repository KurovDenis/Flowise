...
class Evently_SGR_MCP implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Evently SGR Decision'
        this.name = 'eventlySGRMCP'
        this.version = 1.0
        this.type = 'Evently SGR MCP Tool'
        this.icon = 'evently-sgr.svg'
        this.category = 'Tools (MCP)'
        this.description =
            'MCP Server for Evently SGR - provides tool to get available Evently API tools with descriptions and parameters'
    ...
    loadMethods = {
        listActions: async (nodeData: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> => {
            try {
                ...
                console.error('Error listing SGR MCP actions:', error)
                return [
                    {
    ...
    async getTools(nodeData: INodeData, options: ICommonObject): Promise<Tool[]> {
        ...
        try {
            ...
            throw new Error(`Failed to decrypt Evently API credential: ${error instanceof Error ? error.message : String(error)}`)
        }
        ...
        console.log('Evently SGR MCP: Base URL:', baseUrl)
        console.log('Evently SGR MCP: Client ID:', keycloakClientId)
        ...
        try {
            ...
            console.log(`Evently SGR MCP: Initialized with ${tools.length} tools`)
            ...
            throw new Error(`Invalid Evently SGR MCP Server Config: ${error}`)
        }
    }
}

module.exports = { nodeClass: Evently_SGR_MCP }
