#!/usr/bin/env node
import { AttributesMCPServer } from './server/AttributesMCPServer'
...
// Start MCP server
const server = new AttributesMCPServer(apiClient)
server.run().catch((error) => {
    console.error('Failed to start Attributes MCP Server:', error)
    process.exit(1)
})

console.log('✅ Evently Attributes MCP Server started')
