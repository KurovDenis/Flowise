...
class Evently_Attributes_MCP implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Evently Attributes MCP'
        this.name = 'eventlyAttributesMCP'
        this.version = 1.0
        this.type = 'Evently Attributes MCP Tool'
        this.icon = 'evently-attributes.svg'
        this.category = 'Tools (MCP)'
        this.description = 'MCP Server for Evently Attributes - provides 17 attribute-related tools (attribute types, attributes, attribute groups)'
    ...
    loadMethods = {
        listActions: async (nodeData: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> => {
            try {
                ...
                console.error('Error listing Attributes MCP actions:', error)
                return [
                    {
    ...
    async getTools(nodeData: INodeData, options: ICommonObject): Promise<Tool[]> {
        ...
        console.log('Evently Attributes MCP: Base URL:', baseUrl)
        console.log('Evently Attributes MCP: Client ID:', keycloakClientId)
        ...
        const credentialsHash = crypto
            ...
            .digest('hex')
            .substring(0, 16)
        const cacheKey = `evently-attributes-mcp-${workspaceId}-${credentialsHash}`

        // Try to get cached tools
        if (options.cachePool) {
            const cachedResult = await options.cachePool.getMCPCache(cacheKey)
            if (cachedResult) {
                console.log('Evently Attributes MCP: Using cached tools')
        ...
        try {
            ...
            console.log(`Evently Attributes MCP: Initialized with ${tools.length} tools`)
            ...
            throw new Error(`Invalid Evently Attributes MCP Server Config: ${error}`)
        }
    }
}
