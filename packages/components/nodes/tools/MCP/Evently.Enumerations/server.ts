#!/usr/bin/env node
import { EnumerationsMCPServer } from './server/EnumerationsMCPServer'
...
// Start MCP server
const server = new EnumerationsMCPServer(apiClient)
server.run().catch((error) => {
    console.error('Failed to start Enumerations MCP Server:', error)
    process.exit(1)
})

console.log('✅ Evently Enumerations MCP Server started')
