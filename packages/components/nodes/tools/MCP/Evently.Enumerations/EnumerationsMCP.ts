...
class Evently_Enumerations_MCP implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Evently Enumerations MCP'
        this.name = 'eventlyEnumerationsMCP'
        this.version = 1.0
        this.type = 'Evently Enumerations MCP Tool'
        this.icon = 'evently-enumerations.svg'
        this.category = 'Tools (MCP)'
        this.description = 'MCP Server for Evently Enumerations - provides 8 enumeration-related tools (enumerations and values)'
    ...
    loadMethods = {
        listActions: async (nodeData: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> => {
            try {
                ...
                console.error('Error listing Enumerations MCP actions:', error)
                return [
                    {
    ...
    async getTools(nodeData: INodeData, options: ICommonObject): Promise<Tool[]> {
        ...
        console.log('Evently Enumerations MCP: Base URL:', baseUrl)
        console.log('Evently Enumerations MCP: Client ID:', keycloakClientId)
        ...
        const credentialsHash = crypto
            ...
            .digest('hex')
            .substring(0, 16)
        const cacheKey = `evently-enumerations-mcp-${workspaceId}-${credentialsHash}`

        // Try to get cached tools
        if (options.cachePool) {
            const cachedResult = await options.cachePool.getMCPCache(cacheKey)
            if (cachedResult) {
                console.log('Evently Enumerations MCP: Using cached tools')
        ...
        try {
            ...
            console.log(`Evently Enumerations MCP: Initialized with ${tools.length} tools`)
            ...
            throw new Error(`Invalid Evently Enumerations MCP Server Config: ${error}`)
        }
    }
}

module.exports = { nodeClass: Evently_Enumerations_MCP }
