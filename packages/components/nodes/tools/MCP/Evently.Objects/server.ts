#!/usr/bin/env node
import { ObjectsMCPServer } from './server/ObjectsMCPServer'
...
// Start MCP server
const server = new ObjectsMCPServer(apiClient)
server.run().catch((error) => {
    console.error('Failed to start Objects MCP Server:', error)
    process.exit(1)
})

console.log('✅ Evently Objects MCP Server started')
