...
class Evently_Measures_MCP implements INode {
    label: string
    name: string
    version: number
    description: string
    ...
    constructor() {
        this.label = 'Evently Measures MCP'
        this.name = 'eventlyMeasuresMCP'
        this.version = 1.0
        this.type = 'Evently Measures MCP Tool'
        this.icon = 'evently-measures.svg'
        this.category = 'Tools (MCP)'
        this.description =
            'MCP Server for Evently Measures - provides 10 measure-related tools (measure unit groups, measure units)'
    ...
    loadMethods = {
        listActions: async (nodeData: INodeData, options: ICommonObject): Promise<INodeOptionsValue[]> => {
            try {
                ...
                console.error('Error listing Measures MCP actions:', error)
                return [
                    {
    ...
    async getTools(nodeData: INodeData, options: ICommonObject): Promise<Tool[]> {
        ...
        console.log('Evently Measures MCP: Base URL:', baseUrl)
        console.log('Evently Measures MCP: Client ID:', keycloakClientId)
        ...
        const credentialsHash = crypto
            ...
            .digest('hex')
            .substring(0, 16)
        const cacheKey = `evently-measures-mcp-${workspaceId}-${credentialsHash}`

        // Try to get cached tools
        if (options.cachePool) {
            const cachedResult = await options.cachePool.getMCPCache(cacheKey)
            if (cachedResult) {
                console.log('Evently Measures MCP: Using cached tools')
        ...
        try {
            ...
            console.log(`Evently Measures MCP: Initialized with ${tools.length} tools`)
            ...
            throw new Error(`Invalid Evently Measures MCP Server Config: ${error}`)
        }
    }
}

module.exports = { nodeClass: Evently_Measures_MCP }
